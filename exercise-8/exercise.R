...
# Read in the data
pulitzer <- read.csv("/Users/QLUO/Desktop/m11-dplyr/exercise-8/data/pulitzer-circulation-data.csv", stringsAsFactors = FALSE)
...
Pulitzer.Prize.Change <- mutate(pulitzer, diff.new.to.old = pulitzer$Pulitzer.Prize.Winners.and.Finalists..2004.2014 
                                - pulitzer$Pulitzer.Prize.Winners.and.Finalists..1990.2003)

# What publication gained the most pulitzer prizes from 2004-2014?
# Be sure to use the pipe operator! 
best.pulitzer <- filter(pulitzer, pulitzer$Pulitzer.Prize.Winners.and.Finalists..2004.2014 == 
...
change.the.most <- filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 >= 5) %>%
